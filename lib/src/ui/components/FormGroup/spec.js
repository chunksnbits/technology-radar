...
it('renders without crashing', function () {
    var element = shallow(React.createElement(FormGroup, null,
        React.createElement("input", null)));
    expect(element.exists).toBeTruthy();
});
it('throws error if no child is provided', function (done) {
    try {
        shallow(React.createElement(FormGroup, null));
        fail();
    }
    catch (error) {
        done();
...
it('renders label if provided', function () {
    var element = shallow(React.createElement(FormGroup, { label: 'Text' },
        React.createElement("input", null)));
